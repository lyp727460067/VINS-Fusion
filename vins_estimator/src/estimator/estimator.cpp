...
void Estimator::optimization()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        std::cout<<"marginalization_flag == MARGIN_OLD"<<std::endl;
        MarginalizationInfo *marginalization_info = new MarginalizationInfo();
        vector2double();

        if (last_marginalization_info && last_marginalization_info->valid)
        {
            std::cout<<" last_marginalization_info  marginalization_flag == MARGIN_OLD"<<std::endl;
            vector<int> drop_set;
        ...
        {
            out_str<<"befor "<<reinterpret_cast<long>(para_Pose[i]);
            addr_shift[reinterpret_cast<long>(para_Pose[i])] = para_Pose[i - 1];
            out_str<<"after "<<reinterpret_cast<long>(para_Pose[i])<<std::endl;
            if(USE_IMU)
    ...
    else
    {
        ...
        std::cout<<"marginalization_flag == NEWA"<<std::endl;
        if (last_marginalization_info &&
            std::count(std::begin(last_marginalization_parameter_blocks), std::end(last_marginalization_parameter_blocks), para_Pose[WINDOW_SIZE - 1]))
        {

            std::cout<<"last_marginalization_info  marginalization_flag ==NEW"<<std::endl;
            MarginalizationInfo *marginalization_info = new MarginalizationInfo();
            vector2double();
            if (last_marginalization_info && last_marginalization_info->valid)
            {
